...
import pl.dotjabber.sentiment.historical.service.wiki.WikiExtractorService;
...
public class SearchService {
    ...
    @Autowired
    private WikiExtractorService wikiExtractorService;

    @Autowired
    private SentimentService sentimentService;

    public SearchResponse search(String query, boolean wikiIncluded) throws IOException {
        SearchResponse response = new SearchResponse();

        if(wikiIncluded) {
            String wikipediaText = wikiExtractorService.getArticleForTerm(query, true);
            response.addGloss(WordTagger.getClean(wikipediaText));
        }
